...
export async function POST(req: NextRequest) {
  try {
    const body = await req.json() as ComplaintInsert & { category: string }
    ...
    const validCategories = ['infrastructure', 'noise', 'safety', 'sanitation', 'environment', 'other']
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        submitter_name: body.is_anonymous ? 'Anonymous' : (body.submitter_name || '').trim(),
